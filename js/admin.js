...
import { collection, addDoc, deleteDoc, doc, getDocs, query, orderBy } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
export async function cargarPedidosAdmin() {
  ...
  if (pedidos.length===0) { lista.innerHTML=''; show(vacio); return; }
  hide(vacio);
  lista.innerHTML=pedidos.map(p=>`
    <div class="pedido-card-admin">
      ...
      <div class="pedido-admin-items">
        ${p.items.map(i=>`${i.emoji} ${i.nombre} ×${i.cantidad}${i.comentario?` <em>(${i.comentario})</em>`:''} — ${formatPrecio(i.precio*i.cantidad)}`).join('<br>')}
      </div>
    </div>`).join('');
}
...
export async function cargarCumplesAdmin() {
  ...
  if (cumples.length===0) { lista.innerHTML='<p class="vacio-msg">No hay cumpleaños registrados.</p>'; return; }
  ...
  lista.querySelectorAll('.btn-eliminar').forEach(btn=>{
    btn.addEventListener('click',async()=>{ if(confirm('¿Eliminar?')){ await eliminarCumpleanos(btn.dataset.id); cargarCumplesAdmin(); } });
  });
}

export function initFormCumple() {
  document.getElementById('btn-nuevo-cumple').addEventListener('click',()=>show(document.getElementById('form-cumple')));
  document.getElementById('btn-cancelar-cumple').addEventListener('click',()=>hide(document.getElementById('form-cumple')));
...
export async function cargarUsuariosAdmin() {
  ...
  if(usuarios.length===0){lista.innerHTML='<p class="vacio-msg">Sin usuarios.</p>';return;}
  ...
  lista.querySelectorAll('.btn-eliminar').forEach(btn=>{
    btn.addEventListener('click',async()=>{ if(confirm('¿Eliminar usuario?')){ await deleteDoc(doc(db,'usuarios',btn.dataset.id)); cargarUsuariosAdmin(); } });
  });
}

export function initFormUsuario() {
  document.getElementById('btn-nuevo-usuario').addEventListener('click',()=>show(document.getElementById('form-usuario')));
  document.getElementById('btn-cancelar-usuario').addEventListener('click',()=>hide(document.getElementById('form-usuario')));
  ...
  document.getElementById('btn-guardar-usuario').addEventListener('click',async()=>{
    ...
    const rutClean=rutInput.value.replace(/[^0-9kK]/g,'').toUpperCase();
    ...
    document.getElementById('usuario-nombre').value=''; rutInput.value='';
    hide(document.getElementById('form-usuario')); cargarUsuariosAdmin();
  });
}
...
export async function cargarMenuAdmin() {
  ...
  lista.innerHTML='<p style="color:var(--text2);font-size:.85rem">Cargando...</p>';
  let items=[];
  try { items=await getMenuLocalFlat('fuente-alemana'); } catch(e){ lista.innerHTML=`<p class="vacio-msg">Error: ${e.message}</p>`; return; }
  if(items.length===0){ lista.innerHTML='<p class="vacio-msg">Sin ítems. Usa "Importar menú".</p>'; return; }
  ...
  lista.querySelectorAll('.btn-eliminar').forEach(btn=>{
    btn.addEventListener('click',async()=>{ if(confirm('¿Eliminar ítem?')){ await eliminarMenuItem(btn.dataset.id); cargarMenuAdmin(); } });
  });
}
...
export function initImportarMenu() {
  document.addEventListener('click',async(e)=>{
    if(e.target&&e.target.id==='btn-importar-menu'){
      ...
      show(status); status.textContent='Importando menú...'; e.target.disabled=true;
      try{ await importarMenu('fuente-alemana',MENU_FUENTE_ALEMANA); status.textContent=`✓ ${MENU_FUENTE_ALEMANA.length} ítems importados.`; cargarMenuAdmin(); }
